...
class Blip2Dataset(Dataset):
    """
    PyTorch Dataset for Blip2. This class takes a HuggingFace Dataset as input.
    """
...
train_dataset = Blip2Dataset(split_dataset['train'][:10])
validation_dataset = Blip2Dataset(split_dataset['test'][:10])
...
#device_map = infer_auto_device_map(model, max_memory={0:'16GB','cpu':'48GB'})
...
model = Blip2ForConditionalGeneration.from_pretrained(
    checkpoint, load_in_8bit=True, device_map={"": 0}, torch_dtype=torch.float16
)'''
...
class Blip2ModelPLModule(L.LightningModule):
    ...
    def val_dataloader(self):
        return DataLoader(validation_dataset, collate_fn=eval_collate_fn, batch_size=self.batch_size, shuffle=False, num_workers=4)
...
model_module = Blip2ModelPLModule(config, processor, model)
